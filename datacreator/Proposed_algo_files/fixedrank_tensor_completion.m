function [Xsol, infos, problem_description] = fixedrank_tensor_completion(ts, tr, OS)
...
%          weighted matrix completion) and regularization.
    ...
    %end
    problem_description = check_problem_description(problem_description, ts, tr, OS); % Check the problem description;
    ...
    % Weights
    w = 1;%problem_description.weights; % Weighted tensor completion W.*(X - X^start).
    
   
    problem_description.OS = OS;
    ...
    function [g store] = egrad(X, store)
        ...
        g.G = g.G + lambda*X.G;
        %b = g;
        %g = iterative_method(g, X, g, 0.1);
        %problem.M.egrad2rgrad(X,g);
        %g = iterative_method(g, X, b);
    ...
    % Call the solver
    problem_description.X_initial = problem.M.rand();
    X_initial = problem_description.X_initial;
    filen = [num2str(ts),'_',num2str(tr),'_',num2str(OS),'.mat'];
    save(filen, 'problem_description');
...
function checked_problem_description = check_problem_description(problem_description, ts, tr, OS)
    ...
    if isempty(problem_description)...
        ...
        checked_problem_description = get_random_instance(ts,tr,OS);
...
function problem_description = get_random_instance(ts, tr, OS)
...
function show_plots(problem_description, infos)
    ...
    [foldername, filename1, filename2] = create_name(problem_description);
    cd (foldername);
