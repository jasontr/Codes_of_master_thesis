...
% Change log:
    ...
    %no psi
    function rgrad = egradrgrad(X, egrad)
    ...
    %pi
    function rgrad = egrad22rgrad(X, egrad)
        X = prepare(X); % Reuse already computed terms
        ...
        egrad_G1 = reshape(egrad.G, r1, r2*r3); 
        egrad_G2 = reshape(permute(egrad.G, [2 1 3]), r2, r1*r3); 
        egrad_G3 = reshape(permute(egrad.G, [3 1 2]), r3, r1*r2);
% Project onto the horizontal space.
        PU1 = skew((X.U1'*egrad.U1)*X.G1G1t) + skew(X.G1*egrad_G1');
        PU2 = skew((X.U2'*egrad.U2)*X.G2G2t) + skew(X.G2*egrad_G2');
        PU3 = skew((X.U3'*egrad.U3)*X.G3G3t) + skew(X.G3*egrad_G3');
        ...
        rgrad.U1 = egrad.U1 - (X.U1*Omega1);
        rgrad.U2 = egrad.U2 - (X.U2*Omega2);
        rgrad.U3 = egrad.U3 - (X.U3*Omega3);
        ...
        rgrad.G = egrad.G -(-(GOmega1+GOmega2+GOmega3));

    end
    
    function rgrad = egrad222rgrad(X, egrad)
    ...
    M.proj = @projection;
    function etaproj = projeeection(X, eta)
        X = prepare(X);
    ...
    function etaproj = projeection(X, eta)
        X = prepare(X); % Reuse already computed terms
    ...
    %Nuf
    function Y = retrction(X, eta, t)
        if nargin < 3
            t = 1.0;
        end
        
        Y.G = (X.G + t*eta.G);
        Y.U1 = (X.U1 + t*eta.U1); % U factor of Polar factorization
        Y.U2 = (X.U2 + t*eta.U2);
        Y.U3 = (X.U3 + t*eta.U3);
